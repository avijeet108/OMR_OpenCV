...
# UI CODE USING TKINTER

my_w.geometry("400x400")
my_w.title("Optical Mark Recognition")

l1 = tk.Label(my_w,  text='Upload Image to get the Score', font=24,bg='lightgreen' )  
l1.grid(row=0,column=0,columnspan=10,padx=10,pady=20,sticky='ew') 
b1=tk.Button(my_w,text='Upload',command=lambda:my_upload())
b1.grid(row=1,column=0,columnspan=5)
b2=tk.Button(my_w,text='Check Score',command=lambda:my_add())
b2.grid(row=1,column=6,columnspan=5)
b3=tk.Button(my_w,text='Open Webcam',command=lambda:my_web())
b3.grid(row=10,column=3,columnspan=5)
def my_upload(): # show file browser and preview of photo
    f_types = [('All Files', '*.*'), 
             ('JPG', '*.jpg'),
             ('PNG', '*.png')              ]
    global pathImage
    pathImage = filedialog.askopenfilename(filetypes=f_types)
def my_add():
    my_w.quit()
def my_web():
    global webCamFeed
